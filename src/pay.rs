...
use crate::request::{self, Request, Requester};
use crate::response::{
...
pub trait Payer {
    ...
    async fn trade_pay(&self, biz_content: &TradePayBiz) -> AliPayResult<TradePayResponse>;

    async fn trade_precreate(
        &self,
        biz_content: &TradePrecreateBiz,
    ) -> AliPayResult<TradePrecreateResponse>;

    async fn trade_app_pay(&self, biz_content: &TradeAppPayBiz) -> AliPayResult<String>;

    async fn trade_wap_pay(&self, biz_content: &TradeWapPayBiz) -> AliPayResult<String>;

    async fn trade_page_pay(&self, biz_content: &TradePagePayBiz) -> AliPayResult<String>;

    async fn trade_query(&self, biz_content: &TradeQueryBiz) -> AliPayResult<TradeQueryResponse>;

    async fn trade_cancel(&self, biz_content: &TradeCancelBiz) -> AliPayResult<TradeCancelResponse>;

    async fn trade_refund(&self, biz_content: &TradeRefundBiz) -> AliPayResult<TradeRefundResponse>;

    async fn trade_page_refund(
        &self,
        biz_content: &TradePageRefundBiz,
    ) -> AliPayResult<TradePageRefundResponse>;
    async fn trade_fastpay_refund_query(
        &self,
        biz_content: &TradeFastpayRefundQueryBiz,
    ) -> AliPayResult<TradeFastpayRefundQueryResponse>;

    async fn trade_order_settle(
        &self,
        biz_content: &TradeOrderSettleBiz,
    ) -> AliPayResult<TradeOrderSettleResponse>;

    async fn trade_order_settle_query(
        &self,
        biz_content: &TradeOrderSettleQueryBiz,
    ) -> AliPayResult<TradeOrderSettleQueryResponse>;

    async fn trade_royalty_relation_bind(
        &self,
        biz_content: &TradeRoyaltyRelationBindBiz,
    ) -> AliPayResult<TradeRoyaltyRelationBindResponse>;

    async fn trade_royalty_relation_unbind(
        &self,
        biz_content: &TradeRoyaltyRelationUnBindBiz,
    ) -> AliPayResult<TradeRoyaltyRelationUnBindResponse>;

    async fn trade_close(&self, biz_content: &TradeCloseBiz) -> AliPayResult<TradeCloseResponse>;
    async fn async_verify_sign(&self, raw_body: &[u8]) -> AliPayResult<bool>;
}
...
impl Payer for PayClient {
    ...
    async fn trade_create(&self, biz_content: &TradeCreateBiz) -> AliPayResult<TradeCreateResponse> {
        let body = self.do_alipay(biz_content).await?;
...
impl PayClient {
    ...
    pub async fn execute(&self, req: &mut impl Requester) -> AliPayResult<Vec<u8>> {
        ...
        if method == "alipay.trade.precreate" {
            request = client
                .get(format!("{}?{}", self.api_url, payload))
                .build()?;
        } else {
            request = client
    ...
    /// 该方法对返回结果自带同步验签
    pub async fn do_alipay(&self, biz_content: &impl BizContenter) -> AliPayResult<Vec<u8>> {
        // 同步验签
        let sync_verigy_sign = |response: &[u8]| -> AliPayResult<bool> {
        ...
        match biz_content.method().as_str() {
            ...
            _ => {
                ...
                let res = self.execute(&mut request).await?;
    ...
    fn create_clien_page_form(&self, biz: &impl BizContenter) -> AliPayResult<Vec<u8>> {
        ...
        url::form_urlencoded::parse(encode_query.as_bytes())
    ...
    fn create_clien_sdk_request(&self, biz: &impl BizContenter) -> AliPayResult<Vec<u8>> {
